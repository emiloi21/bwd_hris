...
<<<<<<< HEAD
              ...
              $sys_emp_f = $conn->query("SELECT COUNT(*) FROM personnels WHERE sex='Female'")->fetchColumn();
              
              // Active Job Status
              $active_status_query = $conn->query("SELECT e.emp_stat_name, COUNT(p.personnel_id) as total FROM emp_status e LEFT JOIN personnels p ON e.empStat_id = p.empStat_id AND (p.separation_date IS NULL OR TRIM(p.separation_date) = '' OR p.separation_date LIKE '0000-00-00%') WHERE e.status = 'Active' GROUP BY e.empStat_id ORDER BY e.emp_stat_name ASC");
...
<<<<<<< HEAD
            ...
            $male_per_ctr_stmt = $conn->prepare("SELECT COUNT(*) FROM personnels WHERE do_id = :do_id AND sex = 'Male'");
